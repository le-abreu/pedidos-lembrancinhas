...
import { getOrderById, getWorkflowExecutionSummary } from "@/server/services/order-service";
...
export default async function OrderDetailPage({ params, searchParams }: PageProps) {
        ...
        {attachments.map((attachment) => (
          ...
          >
            ...
            <span className="muted">
              {attachment.storedFile.mimeType ?? "application/octet-stream"} | {attachment.storedFile.byteSize} bytes
            </span>
            <span className="muted">
              Enviado em {formatDate(attachment.createdAt)}
              {attachment.storedFile.uploadedBy?.name ? ` por ${attachment.storedFile.uploadedBy.name}` : ""}
            </span>
          </a>
        ))}
      </div>
  ...
  const isExecutor = user.profiles.some((item) => item.profile === UserProfileType.EXECUTOR);
  const canManage = isAdmin || isExecutor;
  const availableUsers = isAdmin ? users : users.filter((item) => item.id === user.id);
  ...
  function canInteract(execution: NonNullable<typeof order>["phaseExecutions"][number]) {
    if (isAdmin) {
      return true;
    }
  ...
  return (
    <div className="page-stack">
      <PageHeader
        ...
        action={
          <div className="table-actions">
            ...
            {user.profiles.some((item) => item.profile === UserProfileType.ADMIN) ? (
              <Link className="primary-button" href={`/orders/${typedOrder.id}/edit`}>
                Editar pedido
              </Link>
            ) : null}
          </div>
      ...
      <PageTabs pathname={`/orders/${typedOrder.id}`} currentTab={currentTab} tabs={[...orderTabs]} />
      ...
      {currentTab === "overview" ? (
        <>
          ...
          <div className="two-column">
            <section className="card page-stack">
              <div className="section-heading">
                ...
                <span className="badge">{typedOrder.active ? "Ativo" : "Inativo"}</span>
              </div>
              <div className="detail-block">
              ...
              <div className="detail-block">
                ...
                <span>{typedOrder.deliveryAddress ?? "Retirada sem endereço informado."}</span>
              </div>
              <div className="detail-block">
            ...
            <section className="card page-stack">
              ...
              {typedOrder.items.map((item) => (
                <div key={item.id} className="detail-block">
                  ...
                  <span>
                    Quantidade: {item.quantity} | Valor unitário: {formatCurrency(item.unitPrice?.toString())}
                  </span>
                  <span>Peso unitário: {formatWeight(item.unitWeight?.toString())}</span>
                  <span>{item.extraDescription ?? item.notes ?? "Sem observação."}</span>
                </div>
              ))}
            </section>
          </div>
          ...
          <section className="card page-stack">
            <div className="section-heading">
              <h3>Arquivos do pedido</h3>
              <span className="badge">{typedOrder.attachments.length}</span>
            </div>
            {canManage || user.profiles.some((item) => item.profile === UserProfileType.CLIENT) ? (
              <form action={uploadOrderAttachment} className="form-grid" encType="multipart/form-data">
                <input type="hidden" name="orderId" value={typedOrder.id} />
                <input type="hidden" name="redirectPath" value={`/orders/${typedOrder.id}`} />
            ...
            {renderAttachmentList(typedOrder.attachments, "Sem arquivos vinculados ao pedido.")}
          </section>
        </>
      ) : null}
      ...
      {currentTab === "workflow" ? (
        <section className="card page-stack">
          ...
          <div className="timeline">
            {executionSummary.executions.map(({ phase, execution, isCurrent }) => (
              <div
                key={phase.id}
              ...
              >
                <div>
                  ...
                  <p className="muted">{phase.description ?? "Sem descrição."}</p>
                  <p className="muted">{phase.guidanceMessage ?? "Sem mensagem orientativa."}</p>
                  <p className="muted">
                    Status da fase: {execution?.status ?? "PENDING"} | Status de destino:{" "}
                    {phase.targetStatus?.name ?? "Não altera"}
                  </p>
                  <p className="muted">
                    Regra de responsável:{" "}
                    {phase.requiresSupplier
                      ? `restrita ao fornecedor ${phase.responsibleSupplier?.name ?? "não definido"}`
                      : "sem fornecedor obrigatório"}
                  </p>
                </div>
                <div className="page-stack">
                  <span className="badge">Fornecedor: {execution?.supplier?.name ?? "-"}</span>
                  <span className="badge">Executor: {execution?.executedByUser?.name ?? "-"}</span>
                  <span className="badge">
                    {execution?.completedAt ? `Concluída em ${formatDate(execution.completedAt)}` : "Em aberto"}
                  </span>
                </div>
              </div>
            ))}
          ...
          <section className="card page-stack">
            <div className="section-heading">
              <div>
                ...
                <p className="muted">Apenas o admin ou o executor vinculado ao fornecedor da fase podem atuar.</p>
              </div>
            ...
            <div className="interaction-grid">
              {typedOrder.phaseExecutions.map((execution: any) => {
                ...
                const canComplete = interactionAllowed && execution.status !== "COMPLETED";

                return (
                  <article
                    key={execution.id}
                    className={`interaction-card ${interactionAllowed ? "allowed" : "locked"}`}
                  >
                    <div className="interaction-header">
                      <div>
                        ...
                        <p className="muted">{execution.phase.description ?? "Sem descrição."}</p>
                      </div>
                    ...
                    <div className="interaction-meta">
                      <span className="badge">
                        ...
                        {execution.phase.requiresSupplier
                          ? execution.phase.responsibleSupplier?.name ?? "Não definido"
                          : "Livre"}
                      </span>
                      <span className="badge">Fornecedor da execução: {execution.supplier?.name ?? "-"}</span>
                      <span className="badge">Executor: {execution.executedByUser?.name ?? "-"}</span>
                    </div>
                    ...
                    <div className="interaction-body">
                      ...
                      <div className="detail-block">
                        ...
                        {renderAttachmentList(execution.attachments ?? [], "Sem arquivo vinculado.")}
                      </div>
                    </div>
                    ...
                    <div className="interaction-footer">
                      ...
                      <div className="table-actions">
                            ...
                            >
                              ...
                              {execution.phase.requiresSupplier ? (
                                <div className="detail-block">
                                  ...
                                  <span>{execution.phase.responsibleSupplier?.name ?? "Não definido"}</span>
                                </div>
                              ) : (
                                <label className="field">
                                  <span>Fornecedor</span>
                                  <select name="supplierId" defaultValue={execution.supplierId ?? ""}>
                              ...
                              {isAdmin ? (
                                <label className="field">
                                  <span>Executor</span>
                                  <select name="executedByUserId" defaultValue={execution.executedByUserId ?? ""}>
                              ...
                              <label className="field">
                                ...
                                <textarea name="comment" rows={4} defaultValue={execution.comment ?? ""} />
                              </label>
                              <button className="primary-button" type="submit">
                                Salvar interação
                              </button>
                        ...
                        {canComplete ? (
                          ...
                          >
                            <form action={advanceOrderPhase} className="form-grid" encType="multipart/form-data">
                              <input type="hidden" name="orderId" value={typedOrder.id} />
                              <input type="hidden" name="phaseId" value={execution.phaseId} />
                              <input type="hidden" name="executionId" value={execution.id} />
                              <input
                                type="hidden"
                                name="redirectPath"
      ...
      {currentTab === "invoice" ? (
        <div className="two-column">
          {canManage ? (
            ...
            >
              <form action={createInvoice} className="form-grid" encType="multipart/form-data">
                <input type="hidden" name="orderId" value={typedOrder.id} />
                <input type="hidden" name="redirectPath" value={`/orders/${typedOrder.id}?tab=invoice`} />
                <label className="field">
          ...
          <section className="card page-stack">
            ...
            {typedOrder.invoices.map((invoice: any) => (
              <div key={invoice.id} className="detail-block">
                <strong>
                  NF {invoice.number} {invoice.series ? `- Série ${invoice.series}` : ""}
                </strong>
                <span>
                  Emissão: {formatDate(invoice.issuedAt)} | Valor: {formatCurrency(invoice.amount.toString())}
                </span>
                {renderAttachmentList(invoice.attachments ?? [], "Sem arquivo vinculado à nota fiscal.")}
